...
namespace CleanCode.LongParameterList
{
    ...
    public class ResolutionsQuery
    {
        public ResolutionsQuery(DateRange dateRange, User user, int locationId, LocationType locationType, int? customerId = null)
        {
            DateRange = dateRange;
            User = user;
    ...
    {
        public IEnumerable<Reservation> GetReservations(ResolutionsQuery query)
        {
        ...
        public IEnumerable<Reservation> GetUpcomingReservations(ResolutionsQuery query)
        {
            if (query.DateRange.DateFrom >= DateTime.Now)
                throw new ArgumentNullException("dateFrom");
            if (query.DateRange.DateTo <= DateTime.Now)
